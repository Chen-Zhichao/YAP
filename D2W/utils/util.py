...
def finalize_cfg_for_mode(cfg, ds_name: str, mode: str):
    ...
    if mode == "w2w_simulation" or mode == "w2w_modeling":
        initial_bow_difference_mean_um = cfg.TOP_INI_BOW_MEAN_um - cfg.BOT_INI_BOW_MEAN_um
        initial_bow_difference_std_um = float(
            np.sqrt(cfg.TOP_INI_BOW_STD_um**2 + cfg.BOT_INI_BOW_STD_um**2)
        )
        cfg.SYSTEM_MAGNIFICATION_MEAN_ppm = (cfg.k_mag * initial_bow_difference_mean_um + cfg.M_0) / 1e6
        cfg.SYSTEM_MAGNIFICATION_STD_ppm = (abs(cfg.k_mag) * initial_bow_difference_std_um) / 1e6
...
def convert_3dblox_to_pad_bitmap(cfg,
    ...
    # The bitmap indices are computed directly from x/y coordinates, so the
    # physical bitmap does not depend on input line order. Avoiding a sorted
    # copy saves a large object-array sort for dense pad layouts.
    with open(_bmap_path, 'r') as f:
        bumpid = 0
        for line in f:
            parts = line.strip().split()
    ...
    # Pad bitmap figures are intentionally not generated by default because
    # matplotlib dominates startup time for large layouts.
